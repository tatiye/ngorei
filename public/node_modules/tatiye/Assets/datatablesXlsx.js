...
export function setdatatablesXlsx(row) {
  

    var primaryKey=row.mKey;
    ...
    var atrIDstorageBac='storage'+row.id;
      ...
      let set=setTables(row.db.token)


          var useToken=useTabelToken(row.db.token);
          var setPackageTabel=useToken.messages[0]['Query']['tabel'];
          var rowsetIData =useHandelIDTabel(setPackageTabel+','+primaryKey);
          //var retheadt = JSON.parse(rowsetIData['tableHead']);
          if (rowsetIData['tableHead']) {
              var settableHead = JSON.parse(rowsetIData['tableHead']);
              $("#headerDatatablessaa0").html(rowsetIData['deskripsi']);
              $("#xlsxcontent").html('');
          } else {
              var settableHead =["No","A1","A2","A3","A4","A5"];
              $("#headerDatatablessaa0").html('Tabel');
              $("#xlsxcontent").html('');
          }

          if (rowsetIData['sheetData']) {
              var heetData = JSON.parse(rowsetIData['sheetData']);
              var setsheetData =heetData.sheetData;
              var setcolumnHide =heetData.columnHide;
              var settabelFooter =heetData.tabelFooter;
              var setcolumnDf =heetData.columnDefs;
          } else {
            var setsheetData =false;
            var setcolumnHide =false;
            var settabelFooter =false;
            var setcolumnDf =false;
          }


      if (settabelFooter==false) {
        var tabelFooter=false 
        var sqtabelFooter=false
      } else {
        var tabelFooter=true;
        var sqtabelFooter=settabelFooter;
      }
                    ...
                    "className": "text-center",
                     ...
                     render: function (data, type, rowData) {
                         ...
                         $.each(row.db.useClick,function(use, menu){
                            var desain=`
                            <a  onclick="useClickTabels(['${menu[0]}','${rowData[Target]}','${menu[2]}','${menu[3]}','${menu[4]}']);" href="javascript:void(0);">
                    ...
                    "className": "text-center",
                     ...
                     render: function (data, type, rowData) {
                         ...
                         $.each(row.db.dropdown,function(use, menu){
                            var desain=`
                            <a class="dropdown-item share"  onclick="useClickTabels(['${menu[0]}','${rowData[Target]}','${menu[2]}','${menu[4]}','${menu[5]}','${menu[6]}']);" href="javascript:void(0);">
                    ...
                    "className": "text-center",
                     render: function (datas, type, rowData) {
                        ...
                        $.each(handelSheet.data,function(uses, label){
                            var desain=`
                                   <li class="menu-item">
                                        <a onclick="useClickTabels(['label','${rowData[Target]}','${row.db.fromAction.storage[4]}','${label['kode']}','${label['deskripsi']}']);" href="javascript:void(0);" class="menu-button">
                         ...
                         $.each(row.db.useDevices,function(use, menu){
                            if (menu[0] =='delete') {
                             var desain=`
                                <li class="menu-item">
                                        <a onclick="useClickTabels(['${menu[0]}','${rowData[Target]}','${menu[2]}','${menu[4]}','${menu[5]}','${menu[6]}']);"href="javascript:void(0);" class="menu-button">
                         ...
                         var setdropdown=`
                           ...
                           <div style="padding-right:230px;border-color: rgb(131 146 165 / 0%);box-shadow: 0 0 8px 2px rgb(28 39 60 / 0%);"class="dropdown-menu">
                            <div style="margin-top:-30px;"" class="menu">
                                    ...
                                    <ul class="menu-list">
                                        <li class="menu-item">
                                             <a onclick="useClick(['archive','${rowData[Target]}','${row.db.fromAction.storage[4]}']);"href="javascript:void(0);" class="menu-button">
                                               <i class="feat feat-save feat-scale-18 mr-10px"></i>
                                               <span>Archive aaaaaa... </span>
                                               <span>CTRL+S </span>
                                              </a>
                                            </li>
                                            <li class="menu-item">
                                               <a  onclick="useClickTabels(['spam','${rowData[Target]}','${row.db.fromAction.storage[4]}']);"href="javascript:void(0);" class="menu-button">
   ...
   var myTable=$(atrIDstorage).DataTable({
    ...
    $('.dataTables_length select').select2({minimumResultsForSearch: Infinity });  
      ...
      if (setcolumnHide) {
        $.each(setcolumnHide,function(use, menu){
     ...
     window.reload=function (key) {
        ...
        $('.dt-buttons a').attr("href", 'javascript:void(0);');
// END setDatatables
};
