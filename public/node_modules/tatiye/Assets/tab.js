import tatiyeNet,{modalEntri,mainRouterId}  from "../es6.js"; 
export function setTab(row) {
 
   var appColom=''; 
    window.ClickRefresh=function (key) {
           if (key=='datatables') {
                $(".table").DataTable().ajax.reload( null, false);
           } else if (key=='vue') {
                window.app.storage(); 
           } else {
            
           }
        };






   if (row.body.insert==true) {
        window.ClickAddTables=function (key) {
            modalEntri(key)
        };
        if (row.body.content=='datatables') {
            var add=`
            <a  class="link-03 ml-10px mr-5px"href="javascript:void(0);" onclick="ClickAddTables('${row.body.key}');" id="mysetmodal">
               <i class="fs-15px ms-Icon ms-Icon--Add" id="add"></i>
             </a>`;
        } else if (row.body.content=='vue') {
            var add=`
            <a  class="link-03 ml-10px mr-5px"href="javascript:void(0);" onclick="ClickAddTables('${row.body.key}');" id="mysetmodal">
               <i class="fs-15px ms-Icon ms-Icon--Add" id="add"></i>
             </a>`;

        } else {
        var add=``;
        }
    } else {
       var add='';
    }
   var headerID =row.grid.header.replace(/[^A-Za-z0-9]/g, '');
      $.post(tatiyeNet.autoload,{
                 "package"      :row.package+'/'+row.grid.route,
                 "key"         :'inikey',
                  "resheader"   :headerID,
                 beforeSend: function() {
                      $("#gridBody"+headerID).html(`
                        <div style="width:200px;margin: auto; margin-top: 200px;" class="col-md-12">
                               <div class="spinner-border text-primary " role="status">
                                 <span class="sr-only">Loading...</span>
                               </div>
                       </div>
                      `); 
                 },
                })
               .done(function (data) {
                  $("#gridBodyTap"+headerID).html(data);  

              });

                   var appGridID      =headerID;
                   var appGridColom   =row.grid.colom;
                   var appGridHeader  =row.grid.header;
                   var tablist='';

    window.ClickTab=function (main) {
         if (row.body.navigation=='package') {
             mainRouterId({
                "spinner"    :row.body.spinner,
                "mainroute"  :'gridBodyTap'+appGridID,
                "autoload"   :row.package+'/'+main,
                "patroutes"  :1,
             },'package')
         } else {
              mainRouter({
                "spinner"    :row.body.spinner,
                "mainroute"  :'gridBodyTap'+appGridID,
                "autoload"   :main,
                "patroutes" :1,
             })
         }
    };



            $.each(row.lengthMenu,function(key1, row1){
                if (row1=='active') {
                    var linkTab=row.grid.route;
                } else {
                    var linkTab=row1;
                }
                var desain=`
                 <li class="nav-item">
                   <a  onclick="ClickTab('${linkTab}');"class="nav-link ${row1}" id="${linkTab}-tab5" data-toggle="tab" href="#home5"aria-controls="home" aria-selected="false">${key1}</a>
                 </li>
                `;
                tablist +=desain;
              
            });
             var desain=`<div id="${appGridID}" class="${appGridColom}" 
                   data-id="${appGridID}"
                   data-colom="${row.grid.colom}"
                   data-header="Header${appGridID}">
                       <div class="setModal">
                       <div id="modalHeader">
                       <div class="
                              setModal-header
                              d-flex
                              align-items-center
                              justify-content-between">
                              <ul id="headerGrid"  class="nav nav-line" role="tablist">${tablist}</ul>
                               <div class="d-flex"> 
                                  <a  class="link-03 ml-10px mr-5px"href="javascript:void(0);">
                                     <i class="fs-15px ms-Icon ms-Icon--ChromeMinimize"></i>
                                  </a>   
                                  <a  onclick="ClickRefresh('${row.body.content}');"class="link-03 ml-10px mr-5px"href="javascript:void(0);">
                                      <i class="fs-15px ms-Icon ms-Icon--Rotate90CounterClockwise"></i>
                                  </a>
                                  ${add}
                                  
                            </div>
                          </div>
                        </div>
                        
                        <div id="gridBodyTap${appGridID}" class="setModal-body"></div>
                      </div>
                      </div>`;
                        appColom +=desain; 
// });
  var template=`
<div class="${row.grid.classRow} pd-t-10">
    <div class="row  row-xs"id="grid">
    ${appColom}
    </div>
</div>
`;
$("#"+row.autoload).html(template);
$("#grid").sortable({
        handle: "#headerGrid",
         cursor: 'move', 
       //   update: function() {
             
       // }                                 
   });
};
