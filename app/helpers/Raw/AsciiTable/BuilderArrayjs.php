<?php 
namespace app\Raw\AsciiTable;
use app\Raw\Ds\Collection;
class BuilderArrayjs
{
    /**
     * @var string
     */
     ...
     * @return void
     */
     public function addRows(array $rows,$header='',$footer='')
    {
        $this->header = $header;
        $this->footer = $footer;
        foreach ($rows as $row) { 
    ...
    public function renderTable() : string
    {
        ...
        if ($this->title === null) {
            ...
            $titlePadding = intdiv(max(0, mb_strwidth($borderTop) - mb_strwidth($this->title)), 1);
            $titleString = str_repeat(' ', $titlePadding) . $this->title . PHP_EOL;
        }


        $tableAsString =$header  . $borderMiddle . PHP_EOL . $body . $this->footer;
     ...
     * @return string
     */
    private function renderRow(RowInterface $row,  $columnNames)
    {
