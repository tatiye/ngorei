...
foreach ($Exvariable as $key => $value) {
  ...
  if ($value[0]=='images') {
    $Exp1[]=array('no'=>'   "'.$myName.'"=>tatiye::val("images", $_FILES["'.$myName.'"]["tmp_name"]   ,"'.$storage['tabel'].'","'.$myName.'",500000),');
    // code...
  } else {
  ...
  $Exp1[]=array('no' =>'    );');
  $Exp1[]=array('no' =>'    $result=$db->file($data)->insert("'.$storage['tabel'].'","'.$nmimg['images'].'");');
  ...
  if ($value[0]=='images') {
    $Exp1[]=array('no'=>'   "'.$myName.'"=>tatiye::val("images", $_FILES["'.$myName.'"]["tmp_name"]   ,"'.$storage['tabel'].'","'.$myName.'",500000,$setId),');
    // code...
  } else {
    $Exp1[]=array('no'=>'   "'.$myName.'"=>tatiye::val("text",$_POST["'.$myName.'"]   ,"2|Wajib diisi"),');
  }
  ...
  if ($value[0]=='images') {
   ...
   $Exp1[]=array('no' =>'    $result=$db->file($data)->update("'.$storage['tabel'].'","id =$setId AND userid=$setUId","'.$nmimg['images'].'","$setId");');
