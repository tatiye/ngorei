...
$setApp = file_get_contents(tatiye::expDir('public/mobile/package.json'));
...
foreach($arr['filePath'] as $key => $value) {
   ...
   foreach ($routeLink as $router => $value) {
      foreach ($value as $key => $row) { 
        $Content = preg_replace("/&#?[a-z0-9]{2,8};/i","",$key); 
        $ContentText =$Text->strreplace([$Content,' ','-']); 
        $tatiyeNet->assign_block_vars($router, array(
           ...
           'F7'     =>'/'.$Text->strreplace([$ContentText,' ','-']).'/',
           'DESC'   =>$row[0],
           'ICON'   =>$row[1],
        ));
        if (!empty($row[2])) {
